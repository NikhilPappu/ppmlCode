...
namespace aby4
{
	...
	Sh4TaskBase* Sh4Task::basePtr()
	{
		return getRuntime().mTasks.tryGet(mIdx);
	}

	Sh4Task Sh4Runtime::addTask(span<Sh4Task> deps, Sh4Task::RoundFunc&& func, std::string&& name)
	{
		if (func)
		{
	...
	Sh4Task Sh4Runtime::addTask(span<Sh4Task> deps, Sh4Task::ContinuationFunc&& func, std::string&& name)
	{
		if (func)
		{
			auto& newTask = mTasks.emplace();
	...
	Sh4Task Sh4Runtime::addClosure(Sh4Task dep)
	{
		...
		if (taskPtr->isClosure())
			return Sh4Task{ this, dep.mIdx, Sh4Task::Closure };
	...
	Sh4Task Sh4Runtime::configureTask(span<Sh4Task> deps, Sh4TaskBase & newTask)
	{
		...
		for (auto& dep : deps)
		{
			...
			if (depPtr)
			{
				...
				for (auto c : depPtr->mUpstreamClosures)
				{
					if (c == depPtr->mIdx && dep.mType == Sh4Task::Closure)
						continue;
	...
	{
		if (mIsActive)
			throw std::runtime_error("The runtime is currently running a different task. Do not call Sh4Task.get() recursively. " LOCATION);
