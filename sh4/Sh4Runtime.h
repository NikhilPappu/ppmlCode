...
namespace aby4
{
    ...
    public:
		...
		//Sh4Task getClosure(std::string anme);
		...
		bool operator==(const Sh4Task& t) const { return mRuntime == t.mRuntime && mIdx == t.mIdx && mType == t.mType; }
    ...
    public:
        ...
        Sh4Runtime(u64 partyIdx, CommPkg& comm)
        {
            init(partyIdx, comm);
        }
		...
		Sh4Task addTask(span<Sh4Task> deps, Sh4Task::RoundFunc&& func, std::string&& name);

		Sh4Task addTask(span<Sh4Task> deps, Sh4Task::ContinuationFunc&& func, std::string&& name);
		...
		//void configureClosure(span<i64> deps, Sh4TaskBase& handle);
